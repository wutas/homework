...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    X1_norm = torch.sum(x_1 ** 2, axis = -1)
    X2_norm = torch.sum(x_2 ** 2, axis = -1)
    distances = torch.exp(-(X1_norm[:,None] + X2_norm[None,:] - 2 * x_1 @ x_2.T)/sigma)
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(torch.max(torch.zeros_like(scores)
                                ,1-labels * scores))
